...
import com.kingdee.abc.result.ServerResponse;
...
public class EmailController {
    ...
    public ServerResponse sendSimpleMail(){
        try {
            mailService.sendSimpleMail("1187970372@qq.com","Malcomszx Email","周大老板，你好！玛德，陪我烟钱！。。。");
        }catch (Exception ex){
            ex.printStackTrace();
    ...
    public ServerResponse htmlEmail(){
        try {
            mailService.sendHtmlMail("1187970372@qq.com","IJPay让支付触手可及","<body style=\"text-align: center;margin-left: auto;margin-right: auto;\">\n"
    ...
    public ServerResponse attachmentsMail(){
        try {
            String filePath = "C:\\Users\\DELL\\Desktop\\Java并发编程的艺术.pdf";
            mailService.sendAttachmentsMail("1187970372@qq.com", "这是一封带附件的邮件", "邮件中有附件，请注意查收！", filePath);
        }catch (Exception ex){
            ex.printStackTrace();
            return ServerResponse.createByError(-1,"邮件发送失败!!");
        }
